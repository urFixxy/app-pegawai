            ...
            <table class="min-w-full divide-y divide-gray-300">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-gray-900">Check In Time</th>
                        <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-gray-900">Check Out Time</th>
                        ...
                        <tr>
                            ...
                            <td class="relative whitespace-nowrap px-3 py-4 text-sm font-medium text-center">
                                <a href="{{ route('attendances.show', $item->id) }}"
                                    class="text-indigo-600 hover:text-indigo-900"><i class="fa-solid fa-eye"></i></a> |
                                <a href="{{ route('attendances.edit', $item->id) }}"
                                    class="text-indigo-600 hover:text-indigo-900"><i class="fa-solid fa-pen-to-square"></i></a> |
                                <form action="{{ route('attendances.destroy', $item->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Yakin ingin menghapus?')"
                                        class="text-red-600 hover:text-red-900"><i class="fa-solid fa-trash"></i></button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
